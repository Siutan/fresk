  ...
  async function fetchLogs(
    ...
    try {
      if (query) {
        ...
        const { data, error } = await pbGet.getLogsByAppId(
          appId,
          currentPage,
          perPage
        );
      ...
      <Table.Body>
        {#each logs as row (row.id)}
          <Table.Row class="border-muted">
            ...
            <Table.Cell>
              <ItemContextMenu
                triggerText={row.app_environment}
                value={row.app_environment}
                rowLabel="app_environment"
                on:add-filter-query={handleAddFilterQuery}
              />
            </Table.Cell>
            <Table.Cell>
              <ItemContextMenu
                triggerText={row.app_version}
                value={row.app_version}
                rowLabel="app_version"
                on:add-filter-query={handleAddFilterQuery}
              />
            </Table.Cell>
