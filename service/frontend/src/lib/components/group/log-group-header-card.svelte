<script lang="ts">
  ...
  const getSearchQuery = async () => {
    if (!group) return "";
    if (!error) {
      return `${group.log_type}+${group.value}`;
    }
    ...
    if (!stacktrace.codeContext) return `${group.log_type}+${group.value}`;
    ...
    return `${group.log_type}+${group.value}+${highlightedLineContent}`;
  };
</script>
  ...
  <Card.Header>
    <div class="flex justify-between items-center">
      ...
      <div class="flex flex-col items-end">
        ...
        <div class="text-sm">{group.value}</div>
      </div>
    </div>
  </Card.Header>
  <Card.Content>
